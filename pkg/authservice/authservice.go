		...
		return connect.NewResponse(&envoy_auth_v3_pb.CheckResponse{
			HttpResponse: &envoy_auth_v3_pb.CheckResponse_DeniedResponse{
				DeniedResponse: &envoy_auth_v3_pb.DeniedHttpResponse{
					...
					Body: "source address is not a socket address: " + req.Msg.GetAttributes().GetSource().GetAddress().String(),
				},
			},
		}), nil
	}
		...
		return connect.NewResponse(&envoy_auth_v3_pb.CheckResponse{
			HttpResponse: &envoy_auth_v3_pb.CheckResponse_DeniedResponse{
				DeniedResponse: &envoy_auth_v3_pb.DeniedHttpResponse{
					...
					Body: fmt.Sprintf("can't look up %s: %v", remoteAddr, err),
				},
			},
		}), nil
	}
		...
		return connect.NewResponse(&envoy_auth_v3_pb.CheckResponse{
			HttpResponse: &envoy_auth_v3_pb.CheckResponse_DeniedResponse{
				DeniedResponse: &envoy_auth_v3_pb.DeniedHttpResponse{
					...
					Body: fmt.Sprintf("node %s is tagged", info.Node.Hostinfo.Hostname()),
				},
			},
		}), nil
	}
			...
			return connect.NewResponse(&envoy_auth_v3_pb.CheckResponse{
				HttpResponse: &envoy_auth_v3_pb.CheckResponse_DeniedResponse{
					DeniedResponse: &envoy_auth_v3_pb.DeniedHttpResponse{
						...
						Body: fmt.Sprintf("can't extract tailnet name from hostname %q", info.Node.Name),
					},
				},
			}), nil
		}
